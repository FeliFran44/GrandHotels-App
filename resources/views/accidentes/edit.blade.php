        ...
        <div class="card-body">
            <form action="{{ route('accidentes.update', $accidente) }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3"><label for="categoria" class="form-label">Categoría</label><select class="form-select" name="categoria" required>@foreach($categorias as $cat)<option value="{{ $cat }}" {{ $accidente->categoria == $cat ? 'selected' : '' }}>{{ $cat }}</option>@endforeach</select></div>
                </div>
                <div class="row">
                    <div class="col-md-6 mb-3"><label for="gravedad" class="form-label">Gravedad</label><select class="form-select" name="gravedad" required>@foreach(($gravedades ?? []) as $g)<option value="{{ $g }}" {{ $accidente->gravedad == $g ? 'selected' : '' }}>{{ $g }}</option>@endforeach</select></div>
